...
class SpecimenModel {
    ...
    mutate(mutationRate: number = 0.05){
            ...
            (t) => {
                ...
                for(let i = 0; i < t.points.length; i++) {
                    if(Math.random() < mutationRate){
                        ...
                        t.points[i][0] = MathHelper.clamp(shiftScaler * shift[0] + t.points[i][0], 0, 1);
                        t.points[i][1] = MathHelper.clamp(shiftScaler * shift[1] + t.points[i][1], 0, 1);
                    }
                }
                ...
                if(Math.random() < mutationRate) {
                    ["red","blue","green"].forEach(
                        (c: string) => {
                            ...
                            (<any>t.color)[c] = MathHelper.clamp((<any>t.color)[c] + shift * shiftScaler, 0, 255);
                        }
                    );
                }
            }
